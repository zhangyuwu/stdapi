...
INT8U  OSSemPost (OS_EVENT *pevent, INT8U *err)
{
#if OS_CRITICAL_METHOD == 3                                /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr;                               
...
#if OS_ARG_CHK_EN > 0
    if (pevent == (OS_EVENT *)0) {                         /* Validate 'pevent'                        */
		RawDebugOutputString("sem: OS_ERR_PEVENT_NULL\n");
		*err = OS_ERR_PEVENT_NULL;
        return (OS_ERR_PEVENT_NULL);
    }
#endif
    if (pevent->OSEventType != OS_EVENT_TYPE_SEM) {        /* Validate event block type                */
		RawDebugOutputString("sem: OS_EVENT_TYPE_SEM\n");
		*err = OS_ERR_EVENT_TYPE;
	...
	RawDebugOutputString("sem: OS_SEM_OVF\n");
	*err = OS_SEM_OVF;
    return (OS_SEM_OVF);
}
/*$PAGE*/
